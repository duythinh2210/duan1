...
function showsp($dssp) {
    ...
    if (is_array($dssp) && count($dssp) > 0) {
        foreach ($dssp as $sp) {
            ...
            $id = isset($sp['id']) ? $sp['id'] : '';
            $hinh = isset($sp['hinh']) ? $sp['hinh'] : 'default.png';
            $id_catalog = isset($sp['id_catalog']) ? $sp['id_catalog'] : 0;
            $ten_sp = isset($sp['ten_sp']) ? $sp['ten_sp'] : 'Unknown';
            $so_luot_xem = isset($sp['so_luot_xem']) ? $sp['so_luot_xem'] : 0;
            $gia = isset($sp['gia']) ? $sp['gia'] : 0;
            $gia_giam = isset($sp['gia_giam']) ? $sp['gia_giam'] : '';

            $html_dssp .= '<div class="pro' . (isset($sp['dac_biet']) ? ' special' : '') . '">
                              <a href="index.php?pg=sproduct&id=' . $id . '">
                                  <img src="layout/img/products/' . $hinh . '" alt="" height="380px;">
                                  ' . ($specialText ? '<div class="special-text">' . $specialText . '</div>' : '') . '
                              </a>
                              <div class="des">
                                  <span class="category">' . $id_catalog . ' - ' . get_name_catalog($id_catalog) . '</span>
                                  ' . ($so_luot_xem ? '<div class="views"><i class="fas fa-eye"></i> ' . $so_luot_xem . '</div>' : '') . '
                                  <a href="index.php?pg=sproduct&id=' . $id . '">
                                      <h5>' . $ten_sp . '</h5>
                                  </a>
                                  <div class="star">
                                  ...
                                  <h4>' . number_format($gia) . ' VNĐ ' . ($gia_giam ? $gia_giam : '') . '</h4>
                              </div>
...
function update_quantity($id, $new_quantity) {
